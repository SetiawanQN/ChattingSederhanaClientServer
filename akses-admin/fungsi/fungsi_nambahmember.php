<?php
	...
	if($jum>0) { // jika terdapat lebih dari 1 baris dengan username yang sama maka akan muncul alert dibawah ini.. dan akan diredirect ke halaman pendaftaran_member
		...
		if($tambah) {
			echo "<script>alert('Pendaftaran akun member berhasil !');location.replace('../halaman_admin.php');</script>"; //pemberitahuan jika penambahan member berhasil dan diredirect ke halaman_admin.php
		} else {
			echo "<script>alert('Maaf, Pendaftaran Akun Member Gagal.');location.replace('../halaman_admin.php');</script>"; //pemberitahuan jika penambahan member gagal dan diredirect kehalaman halaman_admin.php
		}
	}
